...
## v4.4.0 (2026-09-02)
### 修正
- **匯出明細補上借款紀錄**：原本四種匯出都只列支出，完全看不到已記錄的借還款
  - 匯出明細紙本預覽／PDF：在支出明細後新增 LOANS 區塊（日期、借出人 → 借入人、幣別、金額、主幣換算），頁首也顯示借款筆數
  - CSV：支出列表後新增 Loans 區段（Date / Lender / Borrower / Amount / Currency / Base / Note）
  - 複製文字：新增「— 借款紀錄 —」段落，外幣借款附主幣換算
  - 個人帳單 PDF：新增「LOANS · 借款紀錄」表格（你借給／你向誰借），摘要區加入「你借出」「你借入」
- **修正個人帳單 NET 金額未計入借款的問題**：原本 NET 只用「已支付 − 總分擔」計算，導致有借款時與下方結算指示的金額對不上；現在 NET = 已支付 − 總分擔 + 借出 − 借入，與結算結果一致

## v4.3.1 (2026-08-22)
### 新增
- 貨幣清單新增尼泊爾盧比（NPR, Nepalese Rupee）

## v4.3.0 (2026-07-18)
### 新增
- **自由選擇結算幣別（主幣）**：匯率頁新增「主幣（結算幣別）」區塊，點選任一已啟用幣別即可切換主幣
  - 總支出、結算、分帳金額都會改以新主幣換算顯示
  - 切換時會依現有匯率自動換算各幣別對新主幣的匯率；即時匯率模式下會立即重新抓取，確保精度
### 修正
- **修正小額幣別匯率嚴重失準的問題**（例如韓元 KRW 顯示 0.02，與實際約 0.0213 差了約 6%）
  - 根本原因：即時匯率抓取後被四捨五入到小數點後兩位，KRW 這類匯率極小的幣別精度全被砍掉，越南盾 VND 甚至會變成 0
  - 匯率改以「有效位數」處理：儲存保留 6 位有效數字、顯示 4 位有效數字（例如 1 KRW = 0.02132 TWD）
  - 手動匯率輸入框改為可輸入任意小數位（原本 step 0.01 擋掉了 0.0213 這類值）
  - 統計頁 FX 摘要、支出明細、個人帳單 PDF 的參考匯率顯示同步修正

## v4.2.2 (2026-07-09)
### 修正
- 修正旅程列表無法完整滑到底的問題：底部固定的「同步狀態／關於」列會遮住最後一張旅程卡片，改為依固定列高度與安全區域預留底部留白
